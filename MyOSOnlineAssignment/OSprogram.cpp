...
int main()
{
	...
	// 读入文件分配表FAT
	char yn;
	ifstream ffi("FAT.txt");//打开文件FAT.txt
	if (!ffi)
	{
	...
	{			//输入命令，分析并执行命令
		...
		while (1)
		{
			cout << "\n201921003436 尚翰林:";					//显示提示符(本系统总假定是C盘)
			if (dspath)
				cout << curpath.cpath;
			cout << ">";
...
void ExecComd(int k)		//执行命令
{
	...
	case 25:UdTabComd();		//udtab命令，显示被删除文件表(调试程序用)
		break;
	case 26:MoveComd(k);        //move命令
		break;
	case 27:BatchComd(k);		//batch命令
		break;
	case 28:FcComd(k);			//fc命令
		break;
	case 29:ReplaceComd(k);		//replace命令
		break;
	default:cout << "\n命令错:" << comd[0] << endl;
	}
}
...
void HelpComd()				//help命令，帮助信息(显示各命令格式)
{
	...
	cout << "move <文件名> <目录名>                  ——移动文件及改名\n";
	cout << "batch batchfile [s]                     ——批处理\n";
	cout << "fc <文件名1> <文件名2>                  ——比较两个文件。\n";
	cout << "replace <文件名> <目录名>               ——文件替换\n";
}
...
int MdComd(int k)		//md命令处理函数
{
	...
	short i, s, s0, kk;
	char attrib = (char)16, * DirName;//这里的attrib是16 表明是目录
	FCB* p;
	...
	//只需要修改findFCB 的属性值让其查找的时候不限属性就可以了
...
int CloseComd(int k)				//close命令的处理函数：关闭文件 
{
	...
	// 新增关闭当前操作文件（如果在命令行没有指定关闭的目标文件的话）  当前操作文件是指最近一次使用那种将文件放到UOF中的操作
...
int DelComd(int k)			//del(删除文件)命令处理函数
{
	...
	do
	{
		if (delAll == true)
			fcbp = tmp;
		// for循环循环一个目录中存在的所有项  循环次数是一个目录中最多的项数  在这里不设置循环终止条件 让for循环遍历目录中所有的项目
		for (i = 0;; i++, fcbp++)
		{
			ch = fcbp->FileName[0]; //取文件(目录)名的第一个字符
			if (ch == (char)0xe5)   continue;    //当前目录为空目录
...
int ParseCommand(char* p)	//将输入的命令行分解成命令和参数等
{
	...
	for (i = 0; comd[0][i] != '\0'; i++)
		if (comd[0][i] == '/' || comd[0][i]=='.')		//处理cd/，dir/usr等情况
			break;
...
int BatchComd(int k) {
	...
	if (k == 1) {
		...
		short readc;//要读文件的长度
		short pos;	//待读文件从读写指针起始位置
		short b;	//文件的起始盘块
		short offset; //读文件的偏移量
...
int FcComd(int k)
{
	...
	short readc1, readc2;	//指定两个文件的读写字节个数
	...
	// s1 是命令中第一个参数的盘块地址
	// s2 是命令中第二个参数的盘块地址
	// fcbp1 是命令中第一个参数的对应的FCB
	// fcbp2 是命令中第二个参数的对应的FCB
	//如果返回的结果中任意的s值小于1就说明未能找到正确的路径。
	s1 = FindPath(comd[1], attrib, 1, fcbp1);
	if (s1 < 1) {
		cout << "\n文件" << temppath << "不存在，不能读文件。\n";
		return -1;
	}
	...
	if (fcbp1->Fsize != fcbp2->Fsize)cout << "\n" << gFileName1 << "与" << gFileName2 << "长度不同";


	//读写过程 参考readcommand操作




	bnum1 = (pos1 - 1) / SIZE;			// 从文件的第bnum1块开始读
	bnum2 = (pos2 - 1) / SIZE;			// 从文件的第bnum2块开始读
	offset1 = (pos1 - 1) % SIZE;		// 在第bnum1块的偏移位置处开始读
	offset2 = (pos2 - 1) % SIZE;		// 在第bnum2块的偏移位置处开始读
	b1 = fcbp1->Addr;				//取1文件首块号
	b2 = fcbp2->Addr;				//取2文件首块号
	// 参考ReadCommand  这里是寻找读入的第一个盘块
	...
	while (count < min(readc1, readc2)) {
		for (j1 = of1, j2 = of2; j1 < SIZE && j2 < SIZE; ++j1, ++j2) {
			...
			if (Disk[b2][j2] == Disk[b1][j1]) {
				...
				cout << "\n" << gFileName1 << "与文件" << gFileName2 << "在第" << count + 1 << "字节处不同：" << Disk[b1][j1] << "，" << Disk[b2][j2];
				return -1;
			}
		}
		of1 = 0;
